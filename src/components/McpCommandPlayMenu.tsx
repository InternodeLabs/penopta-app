...
export function McpCommandPlayMenu({ command }: { command: string }) {
  ...
  return (
    <div ref={rootRef} className="relative inline-flex shrink-0">
      <button
        type="button"
        className={`grid size-6 place-items-center rounded-md text-muted transition hover:bg-zinc-100 hover:text-foreground focus-visible:bg-zinc-100 focus-visible:text-foreground ${
          open
            ? "bg-zinc-100 text-foreground opacity-100"
      ...
      {open ? (
        ...
        >
          ...
          <button
            type="button"
            role="menuitem"
            className="flex w-full items-center gap-2.5 px-3 py-2 text-left text-sm text-foreground transition hover:bg-zinc-50"
            onClick={copyCommand}
          >
            {copied ? (
