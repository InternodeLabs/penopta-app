...
Set `provider` to `"chatgpt"` or `"claude"` to match the environment you are running in. Pass that same `provider` to every Penopta project-catalog MCP call in this run.

## Discovery/enumeration mechanism (provider = "claude")
...
- If delivery fails, do not pretend the checkpoint advanced — the next run must still load the last checkpoint from Penopta.
